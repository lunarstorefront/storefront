...
test('it can get variant by sku', function () {
    $currency = \Lunar\Models\Currency::factory()->create(['default' => true]);
    $channel = \Lunar\Models\Channel::factory()->create(['default' => true]);
    $language = \Lunar\Models\Language::factory()->create(['default' => true]);
    $customerGroup = \Lunar\Models\CustomerGroup::factory()->create(['default' => true]);
    $taxClass = \Lunar\Models\TaxClass::factory()->create(['default' => true]);
    $productType = \Lunar\Models\ProductType::factory()->create();

    $product = \Lunar\Models\Product::factory()
        ->for($productType)
        ->create();

    $variant = \Lunar\Models\ProductVariant::factory()
        ->for($product)
        ->for($taxClass)
